...
class report_management extends MY_Controller {
	...
	public function drug_consumption($year = "") {
		...
		foreach ($rResult->result_array() as $aRow) {
			$sql = "select '" . $aRow['drug'] . "' as drug_name,'" . $aRow['pack_size'] . "' as pack_size,'" . $aRow['name'] . "' as unit, month(DATE(d_c.period)) as month,d_c.amount as total_consumed from drug_cons_balance d_c where d_c.drug_id='" . $aRow['id'] . "' and d_c.period LIKE '%" . $year . "%' and facility='" . $facility_code . "' order by d_c.period asc";
			...
			for ($i = 1; $i <= 12; $i++) {
				/*$check_month=$i.'-'.$year;
				 //If month is less than ten,add a zero before check
				 //if($check_month<10){
				 //$check_month='0'.$check_month;
				 //}
				 $sql_array=array('07-2013','08-2013');
				 //Chec if month exist in results
				 if(in_array('07-2013',$sql_array)){
				 echo 'OK<br>';
				 die();
	...
	public function getADR($start_date = "") {
		...
		$dyn_table = "<table border='1' cellpadding='5' class='dataTables'>";
		$dyn_table .= "<thead>";
		$dyn_table .= "<tr><th colspan='2'>Patients with Allergy</th><th colspan='2'>Patients without Allergy</th><th>Percentage with Allergy</th><th>Percentage without Allergy</th></tr>";
	...
	public function service_statistics($start_date = "") {
		...
		//Get Total of all patients
		$sql = "SELECT p.current_regimen,count(*) as total FROM patient p LEFT JOIN regimen r ON r.id = p.current_regimen LEFT JOIN regimen_service_type rst ON rst.id = p.service WHERE p.date_enrolled <='$from' AND p.current_status =1 AND p.facility_code = '$facility_code' AND p.current_regimen != '' AND p.current_status != ''";
		$query = $this -> db -> query($sql);
		$results = $query -> result_array();
		$total = $results[0]['total'];

		//Get Totals for each regimen
		$sql = "SELECT count(*) as total, r.regimen_desc,r.regimen_code,p.current_regimen FROM patient p LEFT JOIN regimen r ON r.id = p.current_regimen LEFT JOIN regimen_service_type rst ON rst.id = p.service WHERE p.date_enrolled <='$from' AND p.current_status =1 AND p.facility_code = '$facility_code' AND p.current_regimen != '' AND p.current_status != '' GROUP BY p.current_regimen ORDER BY r.regimen_code ASC";
		$query = $this -> db -> query($sql);
		$results = $query -> result_array();

		if ($results) {
			$dyn_table = "<table id='patient_listing' border='1' cellpadding='5' class='dataTables'><thead><tr>
				<th rowspan='4'>Regimen</th>
				<th colspan='2'>Total</th>
				<th colspan='14'> Adult</th>
				<th colspan='16'> Children </th>
