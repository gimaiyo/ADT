...
class Synchronize_Pharmacy extends MY_Controller {
	...
	public function syncDrugPharma(){
	$drug_transaction_data = Drug_Stock_Movement::getPagedTransactions("34500","1500","1","65","13050","2013-04-06", "1365227976551");
	//var_dump($drug_transaction_data);	
	}

	//Get total of facility info
	public function getTotalServerFacilityInfo() {
		$source = $this -> session -> userdata('facility');
		$number = Facilities::getTotalNumber("0");
		echo $number;
	}

	//Get total of Client Supporters
	public function getTotalServerSupporters() {
		$source = $this -> session -> userdata('facility');
		$number = Supporter::getTotalNumber();
		echo $number;
	}
	...
	//Get the total Number of non_adherence reasons in the server
	public function getTotalServerNonAdherenceReasons() {
		$number = Non_Adherence_Reasons::getTotalNumber();
		echo $number;
	}
	...
	//Get the total Number of drug_sources in the server
	public function getTotalServerDrugSources() {
		$number = Drug_Source::getTotalNumber();
		echo $number;
	}

	//Get the total Number of drug_destinations in the server
	public function getTotalServerDrugDestinations() {
		$number = Drug_Destination::getTotalNumber();
		echo $number;
	}
	...
	public function getDrugs($offset, $limit) {
		...
		foreach ($drugs as $drug) {
			$drug_details = array("id" => $drug -> id, "drug" => $drug -> Drug, "unit" => $drug -> Unit, "pack_size" => $drug -> Pack_Size, "safety_quantity" => $drug -> Safety_Quantity, "generic_name" => $drug -> Generic -> Name, "supported_by" => $drug -> Supporter -> Name, "dose" => $drug -> Drug_Dose -> Name, "duration" => $drug -> Duration, "quantity" => $drug -> Quantity, "source" => $drug -> Source, "enabled" => $drug -> Enabled, "supplied" => $drug -> Supplied);
	...
	public function getDoses($offset, $limit) {
		...
		foreach ($doses as $dose) {
			$dose_details = array("id" => $dose -> id, "name" => $dose -> Name, "value" => $dose -> Value, "frequency" => $dose -> Frequency, "active" => $dose -> Active);
	...
	public function getPatientSources($offset, $limit) {
		...
		foreach ($sources as $source) {
			$source_details = array("id" => $source -> id, "name" => $source -> Name, "active" => $source -> Active);
	...
	public function getRegimens($offset, $limit) {
		...
		foreach ($regimens as $regimen) {
			$regimen_details = array("id" => $regimen -> id, "regimen_code" => $regimen -> Regimen_Code, "regimen_desc" => $regimen -> Regimen_Desc, "category" => $regimen -> Regimen_Category -> Name, "line" => $regimen -> Line, "type_of_service" => $regimen -> Type_Of_Service, "remarks" => $regimen -> Remarks, "source" => $regimen -> Source, "enabled" => $regimen -> Enabled);
	...
	public function getNonAdherenceReasons($offset, $limit) {
		$purposes = Non_Adherence_Reasons::getPagedPurposes($offset, $limit);
		$counter = 0;
		$purposes_array = array();
		foreach ($purposes as $purpose) {
			$purpose_details = array("id" => $purpose -> id, "name" => $purpose -> Name);
	...
	public function getRegimenDrugs($offset, $limit) {
		...
		foreach ($regimen_drugs as $regimen_drug) {
			$regimen_drug_details = array("id" => $regimen_drug -> id, "regimen" => $regimen_drug -> Regimen, "drugcode" => $regimen_drug -> Drugcode, "active" => $regimen_drug -> Active);
	...
	public function getRegimenServiceTypes($offset, $limit) {
		...
		foreach ($service_types as $service_type) {
			$service_type_details = array("id" => $service_type -> id, "name" => $service_type -> Name, "active" => $service_type -> Active);
	...
	public function getDrugTransactions($facility, $offset, $limit) {
		...
		//Retrieve the machine code data from the data passed
		$machine_code_data = $this -> input -> post("machine_codes");
		//$machine_code_data=":65:48031:2012-06-21:,0:155:99:2013-01-09:,1:165:25920:2013-02-08:,12:155:16721:2013-01-10:,15:155:25417:2013-02-06:,2:193:89174:2013-02-25:, ";
		//Check if the client has returned any machine code data. If not, retrieve all relevant data for that facility
		if (strlen($machine_code_data) == 0) {
			...
			foreach ($split_data as $data_element) {
				if (strlen($data_element) > 0) {
					...
					$timestamp = $separated_variables[2];
					$transaction_date = $separated_variables[3];
					//Get all new patients since the last synchronization
					$drug_transaction_data = Drug_Stock_Movement::getPagedTransactions($offset, $limit, $machine_code, $drug, $facility, $transaction_date, $timestamp);
					//Append the results to the array that will be sent back to the client machine
	...
	public function getDrugDestinations($offset, $limit) {
		$sources = Drug_Destination::getPagedSources($offset, $limit);
		$counter = 0;
		$sources_array = array();
		foreach ($sources as $source) {
			$sources_details = array("id" => $source -> id, "name" => $source -> Name, "active" => $source -> Active);
			$sources_array[$counter] = $sources_details;
			$counter++;
		}
		echo json_encode($sources_array);
	}
	
	public function getDrugSources($offset, $limit) {
		$sources = Drug_Source::getPagedSources($offset, $limit);
		$counter = 0;
		$sources_array = array();
		foreach ($sources as $source) {
			$sources_details = array("id" => $source -> id, "name" => $source -> Name, "active" => $source -> Active);
	...
	public function getFacilityInfo($offset, $limit) {
		$facility = $this -> session -> userdata('facility');
		$sources = Facilities::getPagedFacilities($offset, $limit, "0");
		$counter = 0;
		$sources_array = array();
		foreach ($sources as $source) {
			$sources_details = array("id" => $source['id'], "facilitycode" => $source['facilitycode'], "name" => $source['name'], "facilitytype" => $source['facilitytype'], "district" => $source['district'], "weekday_max" => $source['weekday_max'], "weekend_max" => $source['weekend_max'], "county" => $source['county'], "adult_age" => $source['adult_age'], "supported_by" => $source['supported_by'], "service_art" => $source['service_art'], "service_pmtct" => $source['service_pmtct'], "service_pep" => $source['service_pep'], "supplied_by" => $source['supplied_by'],"parent"=>$source['parent']);
			$sources_array[$counter] = $sources_details;
			$counter++;
		}
		echo json_encode($sources_array);
	...
	public function getSupporters($offset, $limit) {
		$facility = $this -> session -> userdata('facility');
		$sources = Supporter::getPagedSupporters($offset, $limit);
		$counter = 0;
		$sources_array = array();
		foreach ($sources as $source) {
			$sources_details = array("id" => $source -> id, "name" => $source -> Name, "active" => $source -> Active);
